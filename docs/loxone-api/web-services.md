...
> **Note**: When using HTTP, the access credentials in the URL are transmitted in plain text!
...
| http:///dev/sys/wsdevice//Reboot | Restart the specified device |
					...
					The device serial number (without colons) must be used in place of .
...
http:///dev/sys/allowhttpiframe/1