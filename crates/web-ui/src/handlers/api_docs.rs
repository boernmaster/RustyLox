...
pub async fn index(State(_state): State<AppState>) -> Html<String> {
    let groups = vec![
        ...
        ApiGroup {
            ...
            endpoints: vec![
                ep("GET", "/api/system/status", "System status, version, MQTT state"),
                ep("GET", "/api/system/log-level", "Get current log level"),
                ep("PUT", "/api/system/log-level", "Set log level (body: {log_level: \"debug\"})"),
            ],
        },
        ApiGroup {
            ...
            endpoints: vec![
                ...
                ep("PUT", "/api/config/general", "Replace full general.json config"),
            ],
        },
        ApiGroup {
            ...
            endpoints: vec![
                ...
                ep("GET",  "/api/miniserver/:id/status","Check connection status"),
                ep("POST", "/api/miniserver/:id/send", "Send HTTP command to Miniserver"),
                ep("POST", "/api/miniserver/:id/get",  "Get values from Miniserver"),
            ],
        },
        ApiGroup {
            ...
            endpoints: vec![
                ...
                ep("POST", "/api/mqtt/subscriptions/reload",    "Reload subscriptions from file"),
                ep("POST", "/api/mqtt/transformers/reload",     "Reload transformers from file"),
            ],
        },
        ApiGroup {
            ...
            endpoints: vec![
                ...
                ep("POST",   "/api/plugins/install",  "Install plugin from ZIP (multipart)"),
                ep("DELETE", "/api/plugins/:md5",     "Uninstall plugin"),
                ep("POST",   "/api/plugins/:md5/upgrade", "Upgrade plugin from ZIP"),
            ],
        },
        ApiGroup {
            ...
            endpoints: vec![
                ...
                ep("POST",   "/api/backup/create",            "Create backup (?include_plugins=true)"),
                ...
                ep("PUT",    "/api/backup/schedule",          "Update backup schedule config"),
            ],
        },
    ];
